...
PostgreSQL database schema for the Datasilk template.
...
```bash
psql -h localhost -U postgres -c "SELECT 'CREATE DATABASE datasilk' WHERE NOT EXISTS (SELECT FROM pg_database WHERE datname = 'datasilk')\gexec"
```

2. Run `deploy.sql` against the database:

```bash
psql -h localhost -U postgres -d datasilk -f deploy.sql
```
